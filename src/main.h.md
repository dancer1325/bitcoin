...
* `class COutPoint {`
  ...
  * `public: unsigned int n;`
    * output's index | transaction
      * Reason: transaction can contain MULTIPLE outpoints
  * `IMPLEMENT_SERIALIZE( READWRITE(FLATDATA(*this)); )`
    * TODO: 
...
* `class CTxIn {`
  ...
  * `public: COutPoint prevout;`
    * == PREVIOUS transaction's output / you are spending now
  * `public CScript scriptSig;`
    * TODO:
  * `public unsigned int nSequence;`
    * uses
      * BEFORE confirming the transaction, being replaced
    * if `nSequence = UINT_MAX;` == final transaction
      * == transaction can NOT be replaced
...
* `class CTxOut {`
  ...
  * `public CScript scriptPubKey;`
    * == public key / 
      * TODO: next input must be able to sign with to claim it. 
...
* `class CTransaction {`
  * == transaction /
    * broadcasted | network
    * contained | blocks 
    * can contain MULTIPLE inputs & outputs
  * `public: int nVersion;`
    * == transaction's format version
    * uses
      * evolve transaction's protocol / backward compatible
  * `public: vector<CTxIn> vin;`
    * TODO:
  * `public vector<CTxOut> vout;`
    * TODO:
...
* `class CBlock {`
  * == MULTIPLE transactions / 
    * collected by Nodes
    * hash them | hash tree
    * scan -- through -- nonce values
      * Reason: block's hash satisfy proof-of-work
  * if the nodes solve the proof-of-work -> node broadcast the block -- to -- everyone
    * == block is added | block chain
  * blocks are appended | disk | "blk0001.dat" /
    * disk's location is handled -- by -- `CBlockIndex`
  * `public int nVersion;`
    * == block's format version
    * uses
      * evolve block's protocol / backward compatible
  * `public uint256 hashPrevBlock;`
    * == PREVIOUS block's hash
  * `public uint256 hashMerkleRoot;`
    * TODO:
  * `public unsigned int nTime;`
    * TODO:
  * `public unsigned int nBits;`
    * TODO:
  * `public unsigned int nNonce;`
    * TODO:
  * 
